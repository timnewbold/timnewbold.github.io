...
We can generate simple median predicted values just as <a href="./ANCOVA.html">we did</a> with simpler linear models (ignore the names associated with these predictions - they refer to the line numbers in the original PREDICTS database). These predicted values are in the log-transformed space as fit in the Poisson GLM.
...
```{r, echo=TRUE,results=TRUE,message=FALSE,warning=FALSE}
nd.FC <- data.frame(LandUse=factor("Primary",levels=levels(hh.sp$LandUse)),
                    ForestCover=seq(from = min(hh.sp$ForestCover),
                                    to = max(hh.sp$ForestCover),
                                    length.out=100))
...
nd.LU <- data.frame(LandUse=factor(
  c("Primary","Pasture"),levels=levels(hh.sp$LandUse)),
  ForestCover=median(hh.sp$ForestCover))
