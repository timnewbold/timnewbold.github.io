...
```{r,echo=TRUE,results=TRUE}
occMod1 <- glm(PresAbs~LandUse+ForestCover,data=hh.sp)
...
```{r,echo=TRUE,results=TRUE}
occMod1a <- glm(PresAbs~LandUse,data=hh.sp)
occMod1b <- glm(PresAbs~ForestCover,data=hh.sp)
...
This model is behaving strangely. If this were a real analysis, we would need to investigate further the issues with this model, but such an investigation is beyond the scope of this session.
...
# Calculte the probability of presence in pasture
...
# Calculate probability of presence where forest cover is zero (i.e., the intercept value)
...
The probability of presence of *P. punicipes* is therefore `r -round(percent.diff.pasture,1)`% lower in pasture than in primary vegetation, a much smaller reduction than we saw for the abundance of this species. Probability of presence is also `r -round(percent.diff.fc,1)`% lower when there is no forest cover compared to when there is 100% forest cover.
...
Our model thus explains `r round(dev.explained*100,1)``% of the variation in the presence/absence of *P. punicipes*.
