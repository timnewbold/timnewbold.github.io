...
First, let's load the _dplyr_, _ggplot2_ and _MASS_ packages:
...
We will work again with the dataset describing the abundance of the parasitoid wasp species *Pimpla punicipes* in sites of different land use in Hawai'i. We will load and process the data as we did in the <a href="./GLMs.html">tutorial on GLMs</a>.
...
hh.species %>% filter(Predominant_land_use %in% c('Primary vegetation','Pasture')) %>%
  ...
  mutate(Site_number = factor(Site_number)) %>%
  filter(Taxon_name_entered=="Pimpla punicipes") %>% droplevels() -> hh.species
...
We know from the <a href="./GLMs.html">session on GLMs</a> that the data describing the abundance of *P. punicipes* are over-dispersed, so we will start straight away by fitting negative binomial GLMs (using the _MASS_ package):

```{r,echo=TRUE,results=TRUE,eval=TRUE,warning=FALSE,message=FALSE}
library(MASS)

mNull <- glm.nb(Measurement~1,data=hh.species)
mLU <- glm.nb(Measurement~LandUse,data=hh.species)
mFC <- glm.nb(Measurement~ForestCover,data=hh.species)
mCombined <- glm.nb(Measurement~LandUse+ForestCover,data=hh.species)
```
