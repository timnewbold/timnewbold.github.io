...
```{r,echo=TRUE,results=TRUE,warning=FALSE,message=FALSE}
ggplot() + 
  geom_point(mapping = aes(x = fitted(m1),y = residuals(m1))) + 
...
```{r, echo=TRUE,results=TRUE,warning=FALSE,message=FALSE}
ggplot(mapping = aes(x = residuals(m1))) +
  geom_histogram() + 
  theme_classic()
```
...
The residuals seem to conform more closely to a normal distributions, although still with a slight deviation.
...
The equality of variance assumption now seems met reasonably well. However, the eagle-eyed among you may also have noticed that the residuals seem to be lower at either end of the range of fitted values. Replotting the fitted versus residual plot, now adding a trend line, highlights this issue.
...
ggplot(mapping = aes(x = fitted(m2),y = residuals(m2))) + 
  ...
  theme_classic()
```
If we re-plot the original relationship between education and fertility, but this time with the log-transformed education values, we can see that there may be a non-linear relationship between education and fertility.

```{r,echo=TRUE,results=TRUE,warning=FALSE,message=FALSE}
ggplot(data = swiss,mapping = aes(x=LogEducation,y=Fertility)) + geom_point() + geom_smooth(method="lm") + theme_classic()
```

Therefore, let's try fitting a quadratic relationship in our model, which is straightforward to do in R.

```{r, echo=TRUE,results=TRUE,warning=FALSE,message=FALSE}
m3 <- lm(formula = Fertility~LogEducation + I(LogEducation^2),data = swiss)
summary(m3)
anova(m3)
```

If we replot the fitted versus residual plot we can see that the residuals are now more closely centered around the zero line, but the equality of variance assumption is now violated again. We won't solve this issue in this session, but it would be important to investigate further in a real study.
...
At the moment, this just gives us the median prediction from our model, but we also need to know the uncertainty in our predictions. For this, we can use the _interval_ parameter. Now, we obtain a data frame with the median prediction ('fit') and the lower and upper 95% confidence intervals ('lwr' and 'upr'). If we repeated our experiment many times from scratch, we would expect 95% of the predictions to fall within these confidence intervals.

```{r, echo=TRUE,results=TRUE,message=FALSE,warning=FALSE}
preds <- predict(m3,interval='confidence')
...
preds <- predict(object=m3,newdata=nd,interval='prediction')
