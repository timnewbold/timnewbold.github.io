...
```{r,echo=TRUE,results=TRUE}
mtcars$LogMPG <- log10(mtcars$mpg)
mtcars$LogHP <- log10(mtcars$hp)
...
```{r, echo=TRUE,results=TRUE,message=FALSE,warning=FALSE}
preds <- predict(object = m3,interval = 'confidence')

mtcars <- cbind(mtcars,preds)

head(mtcars)
```

Now, we can add these predictions to a results graph. Note that we have to back-transform the predictions because our model fit log<sub>10</sub> horsepower.

```{r, echo=TRUE,results=TRUE,message=FALSE,warning=FALSE}
ggplot(data = mtcars,mapping = aes(x = hp,fill=cyl,colour=cyl)) + 
  geom_point(mapping = aes(y = mpg)) + 
  geom_line(mapping = aes(y = 10^(fit))) + 
  # We set colour = NA here, so as not to have lines around our uncertainty ribbons
  geom_ribbon(mapping = aes(ymin = 10^lwr,ymax = 10^upr),alpha=0.2,colour=NA) + 
  guides(colour = guide_legend(title = "Cylinders"),
         fill = guide_legend(title = "Cylinders")) + 
  scale_x_continuous(name = "Horsepower",transform = 'log10') + 
  scale_y_continuous(name = "Fuel efficiency (MPG)",transform = 'log10') + 
  theme_classic()
```
